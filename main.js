...
function updDayNight(dt){
  ...
  if(t<0.21){
    ...
    _ambLight&&(_ambLight.intensity=0.05+f*0.12,_ambLight.color.setHex(0x223366));
    ...
    _ambLight&&(_ambLight.intensity=Math.max(0.05,0.14-f*0.09),_ambLight.color.setHex(0x223366));
...
function _spawnTitanScouts(){
  ...
  const spots=[
    [-108,118],[-120,115],[-132,118],
    [-108,142],[-120,145],[-132,142],
  ];
  spots.forEach(([x,z])=>{
    ...
    m.visible=true;
    scene.add(m);
    // אור ענבר קטן — עיניים זוהרות בחושך
    const sl=new THREE.PointLight(0xff6600,.65,7);sl.position.set(x,2,z);scene.add(sl);
    ...
    const det=new THREE.Mesh(new THREE.SphereGeometry(.32,8,8),new THREE.MeshLambertMaterial({color:0x554400,emissive:0x221a00}));
...
// ── מודל טיטאן — רוטווילר ענק ומפחיד ──
function mkTitan(sz){
  const g=new THREE.Group();
  // ── חומרים ──
