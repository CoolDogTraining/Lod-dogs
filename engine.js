...
const blds=[];
// ── רשימת PointLights של ה-main scene — לכיבוי בכניסה ל-TA (מונע חריגת MAX_LIGHTS) ──
const _mainPLights=[];
...
function buildIndustrialZone(){
  ...
  const sL=new THREE.PointLight(0x9900ee,2,16);sL.position.set(WH_X,WH_H+2.5,WH_Z+WH_D/2);scene.add(sL);_mainPLights.push(sL);
  ...
  [[WH_X-8,8,WH_Z-6],[WH_X+8,8,WH_Z-6],[WH_X,8,WH_Z+4]].forEach(([lx,ly,lz])=>{
    const il=new THREE.PointLight(0xffe8b0,1.1,20);il.position.set(lx,ly,lz);scene.add(il);_mainPLights.push(il);
  ...
  const trigL=new THREE.PointLight(0x8800cc,.8,8);trigL.position.set(WH_X,2,WH_Z-WH_D*.35);scene.add(trigL);_mainPLights.push(trigL);
  ...
  // ספוט ניצוצות
  const spkL=new THREE.PointLight(0xff7700,1.5,10);spkL.position.set(FX-4,6,FZ+3);scene.add(spkL);_mainPLights.push(spkL);
  ...
  [[205,-96],[228,-96],[255,-96],[275,-110],[275,-145],[275,-180],
   [228,-180],[195,-145],[205,-160],[248,-160]].forEach(([lx,lz])=>{
    ...
    const pl=new THREE.PointLight(0xffe090,.95,32);pl.position.set(lx+3,8.5,lz);scene.add(pl);_mainPLights.push(pl);
...
function bldSynagogue(x,z){
  ...
  l1.position.set(x,7,F-6);scene.add(l1);_mainPLights.push(l1);
  const l2=new THREE.PointLight(0xffe8aa,0.7,14);
  l2.position.set(x,12,F-2);scene.add(l2);_mainPLights.push(l2);
...
function bldMosque(x,z){
  ...
  bandLight.position.set(x,4.65,z);scene.add(bandLight);_mainPLights.push(bandLight);
  // כרכוב עליון
  ...
  mosqueLight.position.set(x,4,z-9);scene.add(mosqueLight);_mainPLights.push(mosqueLight);
...
function buildTrainStation(){
  ...
  stLight.position.set(SX,8,SZ);scene.add(stLight);_mainPLights.push(stLight);
...
function updVilla(dt){
  ...
  // מצלמה מאחורי השחקן — חסום בזמן סצנה סינמטית
  if(!G._cinemaMode){
    _vCamTarget.set(
...
function updHUD(){
  ...
  if(_clk){
    ...
    _clk.style.display='block';
    _clk.textContent=`${String(_ch).padStart(2,'0')}:${String(_cm).padStart(2,'0')} ${(G.dayTime>0.75||G.dayTime<0.27)?'🌙':'☀️'}`;
  }
}
...
function drawBigMap(){
  // אם בתל אביב — צייר מפת TA
  if(typeof TA!=='undefined'&&TA.inTA){drawBigMapTA();return;}
  const canvas=document.getElementById('mm-big');
...
function buildLabExterior(){
  ...
  labLight.position.set(x,5.5,z+6);scene.add(labLight);_mainPLights.push(labLight);
...
function _startBigFire(){
  ...
  [[BX,5,BZ+5],[BX-5,4,BZ-3],[BX+5,4,BZ-3],[BX,9,BZ-1]].forEach(([lx,ly,lz])=>{
    const l=new THREE.PointLight(0xff4400,0,45);l.position.set(lx,ly,lz);scene.add(l);fireLights.push(l);_mainPLights.push(l);
  });
  // אור עשן — לבנבן גבוה
  const smokeL=new THREE.PointLight(0x887766,0,30);smokeL.position.set(BX,18,BZ);scene.add(smokeL);_mainPLights.push(smokeL);
...
function _spawnCh7DebrisItems(){
  ...
  scene.add(tagGrp);
  const tagLight=new THREE.PointLight(0xccddff,1.5,6);tagLight.position.set(27,1.8,-123);scene.add(tagLight);_mainPLights.push(tagLight);
  ...
  scene.add(fileGrp);
  const fileLight=new THREE.PointLight(0xff5500,1.2,5);fileLight.position.set(23,1.5,-126);scene.add(fileLight);_mainPLights.push(fileLight);
...
function updCh8(dt){
  ...
  if(G.mission===42){
    ...
    if(insideWH&&!G._ch8WarehouseReached){
      ...
      // ── סינמטי: כניסה למחסן APEX ──
      // מחסן: WH_X=205, WH_Z=-125, גובה ~9, כניסה מצפון (wz+11), קיר דרום (wz-12)
      const wx=wPos.x, wz=wPos.z;
      _playCinema([
        {cam:[wx,    6.5, wz+9],  look:[wx,   3.0, wz-2],  dur:1100, note:'📦 מחסן APEX...'},   // כניסה — wide
        {cam:[wx-10, 4.0, wz],    look:[wx,   2.5, wz-6],  dur:1200},                            // שמאל → מרכז
        {cam:[wx+10, 4.0, wz],    look:[wx,   2.5, wz-6],  dur:1200},                            // ימין → מרכז
        {cam:[wx,    2.5, wz-4],  look:[wx,   4.5, wz-12], dur:1100, note:'🔺 לוגו APEX.'},     // מבט אל קיר הלוגו
        {cam:[wx,    9.0, wz],    look:[wx,   0.0, wz],    dur:900}                              // עילי סיכום
      ],()=>{
...
function _taBuildRoads(){
  ...
  // בעבר הכביש "המשיך לתוך הים" ויצר את הבאג של יבשה/כביש בתוך הים.
  {
    ...
    const rdLen=EAST_EDGE-WEST_EDGE, rdCx=(EAST_EDGE+WEST_EDGE)/2;
    [[80,14],[30,14],[-30,13],[-80,13],[-130,12],[-175,11],[140,11]].forEach(([z,w])=>_taRd(rdCx,z,w,rdLen,false));
  }
  // N-S (x קבוע)
...
function _taBuildApex(){
  ...
  // כניסת גג — מדרגות חוץ (מאחורי הבניין, נגישות)
  const stairBase=new THREE.Mesh(new THREE.BoxGeometry(4,.3,6),new THREE.MeshLambertMaterial({color:0x222228}));
  stairBase.position.set(AX,0.15,AZ+16);_taAddZ(stairBase);
  for(let s=0;s<5;s++){
    const step=new THREE.Mesh(new THREE.BoxGeometry(3.5,.25,0.8),new THREE.MeshLambertMaterial({color:0x2a2a32}));
    step.position.set(AX,0.25+s*.5,AZ+13.5+s*.8);_taAddZ(step);
  }
  // שלט גג ירוק מהבהב
  const roofGlow=new THREE.Mesh(new THREE.SphereGeometry(.4,6,6),new THREE.MeshLambertMaterial({color:0x00ff88,emissive:0x00aa44}));
  roofGlow.position.set(AX,2,AZ+16);_taAddZ(roofGlow);
  const roofLight=new THREE.PointLight(0x00ff88,2,10);roofLight.position.set(AX,2.5,AZ+16);_taAddZ(roofLight);
  G._taApexBldPos={x:AX,z:AZ};
  G._taLabRoofEntry={x:AX,z:AZ+16};// מאחורי הבניין — נגיש לשחקן
  // מעבדה אחורית
...
function _taBuildRabinSquare(){
  ...
  // הערה: מוצב הפיקוד הסגול שהיה כאן (CPX=RX+20,CPZ=RZ-18) הוסר —
  // הוא נבנה כמעט במדויק על כביש ה-E-W ב-z=-80 (רוחב 13, טווח -86.5..-73.5),
  // כך שנראה כמו "חדר סגול באמצע הכביש". הוא גם לא היה מקושר לשום לוגיקת
  // משימה בפועל (G._taRabinCmdPos לא נקרא משום מקום אחר בקוד), אז ההסרה בטוחה לחלוטין.
}
...
function _taBuildPort(){
  ...
  // מחסן נמל פנימי — שומרים ומרסיס
  _taBuildPortWarehouse(PX-22,PZ+10);
}
...
// ════════════════════════════════════════════════
// תחנת משטרה / מוצב APEX — כיכר דיזנגוף
// (אנלוגי לעירייה ב-לוד)
// ════════════════════════════════════════════════
function _taBuildDizengoffHQ(){
  const DX=-30, DZ=-20;
  // מבנה חיצוני
  if(!_taWallTex)_taWallTex=_taMkWallTex(false);
  const body=new THREE.Mesh(new THREE.BoxGeometry(16,10,14),new THREE.MeshStandardMaterial({color:0x3a3040,roughness:.8}));
  body.position.set(DX,5,DZ);body.castShadow=true;_taAddZ(body);
  _taBldList.push({x:DX,z:DZ,w:16,d:14});
  // חלונות
  const eyeM2=new THREE.MeshStandardMaterial({color:0x2a1a3e,emissive:0x1a0a2a,roughness:.1,transparent:true,opacity:.8});
  for(let r=0;r<3;r++) for(let c=-2;c<=2;c++){
    const wn=new THREE.Mesh(new THREE.BoxGeometry(1.8,1.6,.07),eyeM2.clone());
    wn.position.set(DX+c*2.8,2.5+r*2.8,DZ-7.04);_taAddZ(wn);
  }
  // תאורה סגולה — APEX
  const apexL=new THREE.PointLight(0x8800cc,2.5,20);apexL.position.set(DX,6,DZ-8);_taAddZ(apexL);
  // שלט
  const hqC=document.createElement('canvas');hqC.width=300;hqC.height=80;
  const htx=hqC.getContext('2d');htx.fillStyle='#1a001a';htx.fillRect(0,0,300,80);
  htx.fillStyle='#cc00ff';htx.font='bold 32px Arial';htx.textAlign='center';htx.textBaseline='middle';
  htx.fillText('APEX HQ — דיזנגוף',150,40);
  const hqSign=new THREE.Mesh(new THREE.BoxGeometry(7,1.8,.1),new THREE.MeshStandardMaterial({map:new THREE.CanvasTexture(hqC),side:THREE.DoubleSide}));
  hqSign.position.set(DX,9.5,DZ-7.1);_taAddZ(hqSign);
  // חדר פנימי — לובי + מסדרון
  const fl=new THREE.Mesh(new THREE.PlaneGeometry(14,12),new THREE.MeshLambertMaterial({color:0x181020}));
  fl.rotation.x=-Math.PI/2;fl.position.set(DX,.05,DZ);fl._isGround=true;_taAddZ(fl);
  // שולחן קבלה
  const desk=new THREE.Mesh(new THREE.BoxGeometry(6,.9,1.5),new THREE.MeshLambertMaterial({color:0x2a1a30}));
  desk.position.set(DX,.45,DZ+4);_taAddZ(desk);
  // תאורה פנימית
  const intL=new THREE.PointLight(0x9922cc,1.5,10);intL.position.set(DX,4,DZ);_taAddZ(intL);
  G._taDizengoffSq={x:DX,z:DZ};
  G._taDizengoffHQ={x:DX,z:DZ};
}

// ════════════════════════════════════════════════════════════════════
// TA INTERIORS — מעבדת APEX, מחסן נמל, כיכר רבין
// כל חדר: build / enter / exit / upd — תבנית זהה ל-LAB / VILLA
// ════════════════════════════════════════════════════════════════════
...
let _portEnemies=[],_rabinBoss=null,_portColliders=[],_portBarrels=[];

// ════════════════════
// APEX LAB
// ════════════════════
function buildApexLabScene(){
  _apexLabScene=new THREE.Scene();
  _apexLabScene.background=new THREE.Color(0x020410);
  _apexLabScene.fog=new THREE.FogExp2(0x020410,.028);
  _apexLabCam=new THREE.PerspectiveCamera(70,innerWidth/innerHeight,.1,120);
  _apexLabScene.add(_apexLabCam);
  _apexLabFlicker=[];
  const _add=m=>{m.castShadow=true;m.receiveShadow=true;_apexLabScene.add(m);_apexLabObjs.push(m);return m;};
  const _addL=m=>{_apexLabScene.add(m);_apexLabObjs.push(m);return m;};
  const _box=(w,h,d,col,em,x,y,z)=>{const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),new THREE.MeshLambertMaterial({color:col,emissive:em||0}));m.position.set(x,y,z);return _add(m);};
  const _cyl=(r,h,s,col,em,x,y,z)=>{const m=new THREE.Mesh(new THREE.CylinderGeometry(r,r,h,s),new THREE.MeshLambertMaterial({color:col,emissive:em||0}));m.position.set(x,y,z);return _add(m);};
  _addL(new THREE.AmbientLight(0x050a1a,4));
  _addL(new THREE.HemisphereLight(0x001433,0x000814,1.2));
  const mainL=new THREE.PointLight(0x0066ff,5,35);mainL.position.set(0,7,0);_addL(mainL);
  _apexLabFlicker.push({light:mainL,base:5,type:'fluorescent',t:0,period:.06});
  const accentL=new THREE.PointLight(0x00ffff,3,20);accentL.position.set(-8,4,-4);_addL(accentL);
  _apexLabFlicker.push({light:accentL,base:3,type:'pulse',t:0,period:1.4});
  const dangerL=new THREE.PointLight(0xff2200,2.5,14);dangerL.position.set(10,3,6);_addL(dangerL);
  _apexLabFlicker.push({light:dangerL,base:2.5,type:'flicker',t:0,period:.15});
  // חדר 30×28
  _box(30,.3,28,0x0a0c14,0,0,-.15,0);_box(30,.3,28,0x050810,0,0,8.15,0);
  _box(.3,8,28,0x070a18,0,-15,4,0);_box(.3,8,28,0x070a18,0,15,4,0);
  _box(30,8,.3,0x070a18,0,0,4,14);_box(30,8,.3,0x080c1e,0,0,4,-14);
  // דלת כניסה
  _box(4,5,.4,0x1a2040,0x0011ff,0,2.5,-14);_box(3.6,4.6,.2,0x0d1a33,0x0044ff,0,2.3,-13.9);
  // סמני רצפה
  for(let i=-6;i<=6;i+=2){_box(.15,.05,.15,0x002288,0x0033ff,i,.02,12);_box(.15,.05,.15,0x002288,0x0033ff,i,.02,-12);}
  // שולחנות שמאל
  _box(8,.8,2,0x0c0f1e,0,-9,.4,-6);_box(8,.8,2,0x0c0f1e,0,-9,.4,2);
  [-11,-8,-5].forEach(x=>{_box(.05,1.8,1.4,0x000511,0x002255,x,1.7,-6);_box(.05,.05,1.45,0x0033cc,0x0088ff,x,1.8,-6);_box(.05,1.8,1.4,0x000511,0x001133,x,1.7,2);});
  [-10,-7].forEach(x=>{_box(1.2,.1,1.2,0x111830,0,x,.45,-5.2);_box(.1,1.8,.1,0x111830,0,x,.9,-5.6);});
  // מיכלי ניסוי ימין
  [8,11].forEach((x,i)=>{
    _cyl(.9,3.5,12,0x001122,0x004488,x,1.75,-3+i*6);_cyl(.95,.2,12,0x002244,0x0066bb,x,3.45,-3+i*6);_cyl(.85,.05,12,0x000a18,0,x,.05,-3+i*6);
    const fL=new THREE.PointLight(0x00aaff,2.5,8);fL.position.set(x,1.8,-3+i*6);_addL(fL);
  ...
  const kBody=new THREE.Mesh(new THREE.BoxGeometry(.9,1.4,.6),new THREE.MeshLambertMaterial({color:0xe8ecf0,emissive:0x111111}));kBody.position.y=.7;katzG.add(kBody);
  const kHead=new THREE.Mesh(new THREE.SphereGeometry(.35,8,6),new THREE.MeshLambertMaterial({color:0xc8a882}));kHead.position.y=1.6;katzG.add(kHead);
  ...
  // כלובים שמאל אחורי
  for(let i=0;i<3;i++){
    const cx=-11,cz=-8+i*4;
    _box(2.5,2,.1,0x1a2233,0,cx,1,cz);_box(.1,2,2.5,0x223344,0,cx-1.25,1,cz-1.25);_box(.1,2,2.5,0x223344,0,cx+1.25,1,cz-1.25);_box(2.5,.1,2.5,0x1a2233,0,cx,2,cz-1.25);
    for(let b=0;b<5;b++){_box(.06,2,.06,0x334466,0x001133,cx-1+b*.5,1,cz-2.5);}
    const cd=new THREE.Mesh(new THREE.BoxGeometry(.6,.5,.8),new THREE.MeshLambertMaterial({color:0x8b6914}));cd.position.set(cx,.25,cz-1);_add(cd);
  }
  // צינורות תקרה
  for(let x=-12;x<=12;x+=6){_cyl(.12,.12,28,6,0x0d1525,0x001133,x,7.8,0);const pL=new THREE.PointLight(0x0033aa,1,8);pL.position.set(x,7.2,0);_addL(pL);}
}

function enterApexLab(){
  G.paused=true;
  fadeOut(()=>{
    ...
    showN('⚗️ מעבדת APEX. הריח כבד. משהו חי כאן.');
    G.paused=false;fadeIn();
  });
}
function exitApexLab(){
  G.paused=true;
  fadeOut(()=>{
...
function updApexLab(dt){
  if(!APEX_LAB.inLab||G.paused||G.dlgOpen||G.cutOpen)return;
  _apexLabT+=dt;APEX_LAB.enterGrace=Math.max(0,APEX_LAB.enterGrace-dt);
  if(G.atkCD>0)G.atkCD-=dt;
  _apexLabFlicker.forEach(fl=>{
    fl.t+=dt;const L=fl.light;if(!L)return;
    switch(fl.type){
      case'fluorescent':if(fl.t>fl.period){fl.t=0;fl.period=.04+Math.random()*.08;L.intensity=Math.random()<.04?0:fl.base*(.85+Math.random()*.3);}break;
      case'pulse':L.intensity=fl.base*(.5+.5*Math.sin(fl.t*Math.PI*2/fl.period));break;
      case'flicker':if(fl.t>fl.period){fl.t=0;fl.period=.08+Math.random()*.25;L.intensity=Math.random()<.1?0:fl.base*(.6+Math.random()*.7);}break;
      case'bubble':L.intensity=fl.base*(.7+.3*Math.sin(fl.t*3.1+fl.base));break;
      case'crt':L.intensity=fl.base*(.8+.2*Math.sin(fl.t*7));break;
    }
  });
  if(_apexKatzMesh)_apexKatzMesh.rotation.y=Math.PI+Math.sin(_apexLabT*.4)*.15;
  const spd=G.dogs[G.dog].spd;
  _vFwd.set(-Math.sin(G.yaw),0,-Math.cos(G.yaw));_vRgt.set(Math.cos(G.yaw),0,-Math.sin(G.yaw));
  let ix=0,iz=0;
  if(G.keys['KeyW']||G.keys['ArrowUp']){ix+=_vFwd.x;iz+=_vFwd.z;}
  if(G.keys['KeyS']||G.keys['ArrowDown']){ix-=_vFwd.x;iz-=_vFwd.z;}
  if(G.keys['KeyA']||G.keys['ArrowLeft']){ix-=_vRgt.x;iz-=_vRgt.z;}
  if(G.keys['KeyD']||G.keys['ArrowRight']){ix+=_vRgt.x;iz+=_vRgt.z;}
        ...
        showCut('ch9_katz_appears',()=>{
          showCut('ch9_katz_truth',()=>{
            ...
            setTimeout(()=>{
              ...
              if(TA.inTA)[[ap.x-14,ap.z-8],[ap.x+14,ap.z-8],[ap.x-8,ap.z-16],[ap.x+8,ap.z-16]].forEach(([ex,ez])=>_spawnTAEnemy(ex,ez));
              showN('⚔️ APEX רץ לנמל!');G.mission=59;updateMissionHUD();updateNavArrow();saveGame();showN('📋 '+MISSIONS[59].txt);
            },800);
          });
        });
      });
...
// PORT WAREHOUSE
// ════════════════════
function buildPortWHScene(){
  _portWHScene=new THREE.Scene();_portWHScene.background=new THREE.Color(0x050810);_portWHScene.fog=new THREE.FogExp2(0x050810,.02);
  _portWHCam=new THREE.PerspectiveCamera(70,innerWidth/innerHeight,.1,140);_portWHScene.add(_portWHCam);
  const _add=m=>{m.castShadow=true;m.receiveShadow=true;_portWHScene.add(m);_portWHObjs.push(m);return m;};
  const _addL=m=>{_portWHScene.add(m);_portWHObjs.push(m);return m;};
  const _box=(w,h,d,col,em,x,y,z)=>{const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),new THREE.MeshLambertMaterial({color:col,emissive:em||0}));m.position.set(x,y,z);return _add(m);};
  const _cyl=(r,h,s,col,em,x,y,z)=>{const m=new THREE.Mesh(new THREE.CylinderGeometry(r,r,h,s),new THREE.MeshLambertMaterial({color:col,emissive:em||0}));m.position.set(x,y,z);return _add(m);};
  _addL(new THREE.AmbientLight(0x0d0a05,3.5));_addL(new THREE.HemisphereLight(0x1a1205,0x050408,1));
  const l1=new THREE.PointLight(0xff8833,4,32);l1.position.set(-8,8,0);_addL(l1);
  const l2=new THREE.PointLight(0xff6600,3.5,26);l2.position.set(8,8,-5);_addL(l2);
  const l3=new THREE.PointLight(0x4488ff,2,20);l3.position.set(0,5,14);_addL(l3);
  const l4=new THREE.PointLight(0xff8833,3,22);l4.position.set(-15,8,12);_addL(l4);
  const l5=new THREE.PointLight(0x4488ff,2.2,20);l5.position.set(15,8,-13);_addL(l5);
  // חדר מוגדל: 42×34 (היה 34×28) — יותר מקום לתנועה ולתחושת מחסן אמיתי
  _box(42,.3,34,0x0d0c09,0,0,-.15,0);_box(42,.3,34,0x0a0905,0,0,10.15,0);
  _box(.3,10,34,0x0c0b08,0,-21,5,0);_box(.3,10,34,0x0c0b08,0,21,5,0);
  _box(42,10,.3,0x0c0b08,0,0,5,17);_box(42,10,.3,0x0d0c08,0,0,5,-17);
  _box(4,5,.3,0x1a1510,0x883300,0,2.5,-16.9);
  [[-10,1,4],[-10,2.3,4],[-10,1,-5],[-10,2.3,-5],[-5,1,0],[6,1,8],[6,2.3,8],[10,1,3],[10,2.3,3],[10,3.6,3],[10,1,-8],[10,2.3,-8],[2,1,10],[-3,1,-9]].forEach(([x,y,z])=>{
    _box(2.4,1.2,2.4,[0x3d2e1c,0x2e2218,0x4a3825][Math.floor(Math.random()*3)],0,x,y,z);
    _box(2.42,.15,2.42,0x002266,0x004499,x,y+.55,z);
  });
  // ── תוספת: מכולות שילוח בשוליים החדשים, לתחושת מחסן גדול ומלא ──
  [[-18,-13],[-18,13],[18,-13],[18,12]].forEach(([x,z],i)=>{
    const cCol=[0x8a3a2a,0x2a5a6a,0x6a5a2a,0x3a5a2a][i];
    _box(3.2,3,7.5,cCol,0,x,1.5,z);
    _box(3.25,.1,7.55,0x111111,0,x,3.02,z);
    for(let r=0;r<3;r++)_box(3.22,.06,.3,0x000000,0,x,.6+r*1,z+3.7);
  });
  // ── תוספת: גשר/גלריה עילית מעל מרכז החדש — עיטור ותחושת גובה (לא הלוכה, קישוטי) ──
  _box(6,.3,30,0x151516,0,-2,6.4,0);
  [-15,-8,-1,6,13].forEach(z=>{_box(.15,1.1,.15,0x232323,0,-5,7.2,z);_box(.15,1.1,.15,0x232323,0,1,7.2,z);});
  for(let z=-14;z<=14;z+=4){_box(6.2,.1,.1,0x1a1a1a,0,-2,6.95,z);}
  _cyl(.15,.15,20,8,0x1a1a1a,0,-12,9.4,0);_cyl(.15,.15,20,8,0x1a1a1a,0,8,9.4,0);
  for(let x=-12;x<=8;x+=10){const hL=new THREE.PointLight(0xffaa44,1.4,10);hL.position.set(x,8.6,0);_addL(hL);}
  // שדרוג: רשימת מכשולים (תיבות/מכולות) שאויבים ינווטו סביבן בלי להיתקע
  _portColliders=[[-10,4],[-10,-5],[-5,0],[6,8],[10,3],[10,-8],[2,10],[-3,-9],
    [-18,-13,3.4,7.7],[-18,13,3.4,7.7],[18,-13,3.4,7.7],[18,12,3.4,7.7]]
    .map(([x,z,w,d])=>({x,z,w:w||2.6,d:d||2.6}));
  ...
  _box(5,.3,4,0x151310,0,-13,4,-5);
  _portEnemies=[];
  // ── חביות נפץ — מכניקת סביבה: הרס מטווח קרוב פוגע בכל אויב סמוך ──
  _portBarrels=[];
  [[-3,4],[9,-3],[-13,9],[13,7]].forEach(([x,z])=>_spawnPortBarrel(x,z));
}
function _spawnPortBarrel(x,z){
  if(!_portWHScene)return;
  const g=new THREE.Group();
  const body=new THREE.Mesh(new THREE.CylinderGeometry(.55,.6,1.15,10),new THREE.MeshLambertMaterial({color:0xaa1a10,emissive:0x220000}));
  body.position.y=.58;g.add(body);
  const band=new THREE.Mesh(new THREE.CylinderGeometry(.62,.62,.12,10),new THREE.MeshLambertMaterial({color:0x1a1a1a}));
  band.position.y=.58;g.add(band);
  const cap=new THREE.Mesh(new THREE.CylinderGeometry(.4,.4,.06,10),new THREE.MeshLambertMaterial({color:0xffcc00,emissive:0x442200}));
  cap.position.y=1.18;g.add(cap);
  g.position.set(x,0,z);_portWHScene.add(g);_portWHObjs.push(g);
  const b={mesh:g,x,z,hp:24,mhp:24,dead:false};
  _portBarrels.push(b);return b;
}
function _explodePortBarrel(b,nx,nz){
  if(!_portWHScene||b.dead)return;
  b.dead=true;b.mesh.visible=false;
  const flash=new THREE.PointLight(0xff6622,8,12);flash.position.set(b.x,1.2,b.z);_portWHScene.add(flash);_portWHObjs.push(flash);
  setTimeout(()=>{_portWHScene&&_portWHScene.remove(flash);},220);
  haptic([60,30,60]);screenShake(0.4,0.3);
  showDmg(b.x,2,b.z,'💥');
  _portEnemies.forEach(e=>{
    if(e.dead)return;
    const dd=Math.hypot(e.x-b.x,e.z-b.z);
    if(dd<5){
      const dmg=Math.round(70*(1-dd/5));
      e.hp-=dmg;showDmg(e.x,2,e.z,dmg);
      if(e.hp<=0){e.dead=true;e.mesh.visible=false;addXP(30);G.coins+=15;updCoins();sCapture();}
    }
  });
  if(Math.hypot(nx-b.x,nz-b.z)<2.6)dmgPlayer(18);
}
function _spawnPortEnemy(x,z){
  if(!_portWHScene)return;
  const g=new THREE.Group();
  const body=new THREE.Mesh(new THREE.BoxGeometry(1,1.2,.8),new THREE.MeshLambertMaterial({color:0x1a1006}));body.position.y=.6;g.add(body);
  const head=new THREE.Mesh(new THREE.BoxGeometry(.7,.7,.7),new THREE.MeshLambertMaterial({color:0x1a1006}));head.position.y=1.55;g.add(head);
  const helm=new THREE.Mesh(new THREE.BoxGeometry(.78,.45,.78),new THREE.MeshLambertMaterial({color:0x2a1f0a,emissive:0x110800}));helm.position.y=1.8;g.add(helm);
  g.position.set(x,0,z);_portWHScene.add(g);_portWHObjs.push(g);
  // שדרוג: state machine מלא (patrol/chase/search) — בדיוק כמו אויבי ת"א/לוד, במקום alert גורף לפי טווח
  const e={mesh:g,x,z,hp:60,mhp:60,spd:3.5,atk:12,atkT:0,dead:false,alert:16,
    state:'patrol',homeX:x,homeZ:z,patAng:Math.random()*Math.PI*2,patT:2+Math.random()*3,
    lastSeenX:x,lastSeenZ:z,searchT:0};
  _portEnemies.push(e);return e;
}
function enterPortWH(){
  G.paused=true;
  fadeOut(()=>{
    if(!_portWHScene)buildPortWHScene();
    PORT_WH.inWH=true;PORT_WH.playerX=0;PORT_WH.playerZ=10;PORT_WH.enterGrace=2;G.yaw=Math.PI;
    if(_portWHCam){_portWHCam.position.set(0,5,15);_portWHCam.lookAt(0,1,0);}
    if(taScene)taScene.remove(PB);else scene.remove(PB);
    _portWHScene.add(PB);PB.position.set(0,0,10);_portEnemies=[];
    [[-9,2],[9,1],[0,-5],[-7,-9],[8,-8],[-16,-11],[16,10]].forEach(([x,z])=>_spawnPortEnemy(x,z));
    showN('🚢 מחסן נמל APEX. אויבים בפנים. זהירות — יש שם גם חביות נפץ.');
    G.paused=false;fadeIn();
  });
}
function exitPortWH(){
  ...
  fadeOut(()=>{
    ...
    _portWHObjs.length=0;_portEnemies=[];_portBarrels=[];_portWHScene=null;_portWHCam=null;
    G.paused=false;fadeIn();
  });
}
function updPortWH(dt){
  if(!PORT_WH.inWH||G.paused||G.dlgOpen||G.cutOpen)return;
  PORT_WH.enterGrace=Math.max(0,PORT_WH.enterGrace-dt);if(G.atkCD>0)G.atkCD-=dt;
  const spd=G.dogs[G.dog].spd;
  _vFwd.set(-Math.sin(G.yaw),0,-Math.cos(G.yaw));_vRgt.set(Math.cos(G.yaw),0,-Math.sin(G.yaw));
  let ix=0,iz=0;
  if(G.keys['KeyW']||G.keys['ArrowUp']){ix+=_vFwd.x;iz+=_vFwd.z;}
  ...
  if(G.joy.on){ix+=_vFwd.x*(-G.joy.dy)+_vRgt.x*G.joy.dx;iz+=_vFwd.z*(-G.joy.dy)+_vRgt.z*G.joy.dx;}
  const iln=Math.hypot(ix,iz)||1;
  let nx=PORT_WH.playerX+(ix/iln)*spd*dt,nz=PORT_WH.playerZ+(iz/iln)*spd*dt;
  // גבולות מותאמים לחדר המוגדל (42×34, במקום 34×28)
  nx=Math.max(-20,Math.min(20,nx));nz=Math.max(-16.5,Math.min(16.5,nz));
  PORT_WH.playerX=nx;PORT_WH.playerZ=nz;PB.position.set(nx,0,nz);
  if(Math.abs(ix)>.01||Math.abs(iz)>.01){walkT+=dt*8;dogLegs.forEach(lg=>{lg.node.rotation.x=Math.sin(walkT+lg.ph)*.38;});if(dogModel){const b=dogModel._baseY||.25;dogModel.position.y=b+Math.abs(Math.sin(walkT))*.09;}PB.rotation.y=Math.atan2(-ix,-iz);}
  if(_portWHCam&&!G._cinemaMode){const sz=G.dog==='momo'?.58:1,cd=8,ch=4+G.pitch*6;_vCamTarget.set(nx+Math.sin(G.yaw)*cd,1.1*sz+ch,nz+Math.cos(G.yaw)*cd);_portWHCam.position.lerp(_vCamTarget,.1);_portWHCam.lookAt(nx,1.1*sz+.7,nz);}
  if(G.atkCD<=0&&G.keys['KeyF']){G.atkCD=G.dog==='zippo'?.28:.5;G._portAtk=true;}
  // ── AI: patrol → chase → search, זהה לתבנית אויבי ת"א/לוד (canSeePlayer+alertNearby) ──
  // (במקום ה"alert גורף אם dd<18" הישן, שהפך כל קרב לריצה ישירה חסרת אתגר)
  _portEnemies.forEach(e=>{
    if(e.dead)return;
    const dd=Math.hypot(e.x-nx,e.z-nz);
    const sees=canSeePlayer(e,nx,nz,_portColliders)||dd<5;
    if(sees){
      if(e.state!=='chase'){alertNearby(e,nx,nz,_portEnemies);if(e.state==='patrol')showN('👁️ מאבטח גילה אותך!');}
      e.state='chase';e.lastSeenX=nx;e.lastSeenZ=nz;e.searchT=6;
    } else if(e.state==='chase'){
      e.searchT-=dt;if(e.searchT<=0)e.state='search';
    }
    if(e.state==='chase'||e.state==='search'){
      const tx=e.state==='chase'?nx:e.lastSeenX, tz=e.state==='chase'?nz:e.lastSeenZ;
      const ang=Math.atan2(tx-e.x,tz-e.z);
      const mspd=e.state==='search'?e.spd*.55:e.spd;
      let enx=e.x+Math.sin(ang)*mspd*dt,enz=e.z+Math.cos(ang)*mspd*dt;
      let blkX=false,blkZ=false;
      ...
      if(e.state==='search'&&e.searchT>0){e.searchT-=dt;if(e.searchT<=0)e.state='patrol';}
      else if(e.state==='search'&&Math.hypot(e.x-e.lastSeenX,e.z-e.lastSeenZ)<1.2)e.state='patrol';
    } else {
      // סיור אקראי חלק סביב נקודת הבית
      e.patT-=dt;
      if(e.patT<=0){e.patAng=Math.random()*Math.PI*2;e.patT=2+Math.random()*3;}
      const px2=e.homeX+Math.sin(e.patAng)*4,pz2=e.homeZ+Math.cos(e.patAng)*4;
      const dx=px2-e.x,dz=pz2-e.z,l=Math.hypot(dx,dz)||1;
      if(l>1){e.x+=dx/l*(e.spd*.3)*dt;e.z+=dz/l*(e.spd*.3)*dt;e.mesh.position.set(e.x,0,e.z);e.mesh.rotation.y+=(Math.atan2(dx,dz)-e.mesh.rotation.y)*.06;}
    }
    e.atkT=Math.max(0,e.atkT-dt);if(dd<2.2&&e.state==='chase'&&e.atkT<=0){dmgPlayer(e.atk);e.atkT=1.2;haptic([25,10,25]);}
  ...
  // ── חביות נפץ: מכה קרובה פוצצת אותן ופוגעת בכל אויב ברדיוס (וגם בשחקן אם קרוב מדי) ──
  if(G._portAtk){
    for(const b of _portBarrels){
      if(b.dead)continue;
      if(Math.hypot(b.x-nx,b.z-nz)<3.2){
        G._portAtk=false;const dog=G.dogs[G.dog];const dmg=Math.round(dog.pow*10*(1+dog.lv*.1));
        b.hp-=dmg;
        if(b.hp<=0)_explodePortBarrel(b,nx,nz);
        break;
      }
    }
  }
  G._portAtk=false;
  if(PORT_WH.enterGrace<=0&&_portEnemies.length>0&&_portEnemies.every(e=>e.dead)){
    if(!G._portWHCleared){G._portWHCleared=true;showN('✅ המחסן נוקה!');setTimeout(()=>{exitPortWH();setTimeout(()=>{G.mission=60;updateMissionHUD();updateNavArrow();saveGame();showN('📋 '+MISSIONS[60].txt);},900);},1500);}
  }
  if(PORT_WH.enterGrace<=0&&nz>12)exitPortWH();
}

// ════════════════════
// RABIN SQUARE
// ════════════════════
function buildRabinScene(){
  _rabinScene=new THREE.Scene();_rabinScene.background=new THREE.Color(0x0a0c18);_rabinScene.fog=new THREE.FogExp2(0x0a0c18,.018);
  _rabinCam=new THREE.PerspectiveCamera(70,innerWidth/innerHeight,.1,160);_rabinScene.add(_rabinCam);
  const _add=m=>{m.castShadow=true;m.receiveShadow=true;_rabinScene.add(m);_rabinObjs.push(m);return m;};
  const _addL=m=>{_rabinScene.add(m);_rabinObjs.push(m);return m;};
  const _box=(w,h,d,col,em,x,y,z)=>{const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),new THREE.MeshLambertMaterial({color:col,emissive:em||0}));m.position.set(x,y,z);return _add(m);};
  const _cyl=(r,h,s,col,em,x,y,z)=>{const m=new THREE.Mesh(new THREE.CylinderGeometry(r,r,h,s),new THREE.MeshLambertMaterial({color:col,emissive:em||0}));m.position.set(x,y,z);return _add(m);};
  _addL(new THREE.AmbientLight(0x08080f,3));_addL(new THREE.HemisphereLight(0x0d0d1f,0x050508,1));
  const sL1=new THREE.PointLight(0xfff0cc,5,35);sL1.position.set(-15,8,0);_addL(sL1);
  const sL2=new THREE.PointLight(0xfff0cc,5,35);sL2.position.set(15,8,0);_addL(sL2);
  const cL=new THREE.PointLight(0x8844ff,4,20);cL.position.set(0,4,0);_addL(cL);
  // רצפה
  for(let x=-18;x<=18;x+=2)for(let z=-18;z<=18;z+=2){_box(1.9,.15,1.9,((x+z)/2%2===0)?0x1a1c28:0x14161f,0,x,-.08,z);}
  _box(44,.3,44,0x0a0c18,0,0,-.2,0);
  _box(.5,4,44,0x12141e,0,-22,2,0);_box(.5,4,44,0x12141e,0,22,2,0);_box(44,4,.5,0x12141e,0,0,2,-22);_box(44,4,.5,0x12141e,0,0,2,22);
  // מזרקה
  _cyl(5,.4,16,0x1c1e2c,0x110f1e,0,.2,0);_cyl(4.8,.1,16,0x003344,0x005566,0,.35,0);_cyl(.5,3,8,0x1a1c2a,0x110f22,0,1.7,0);
  for(let a=0;a<6;a++){const ang=a/6*Math.PI*2;const wl=new THREE.PointLight(0x44aaff,1.5,6);wl.position.set(Math.cos(ang)*2,1.5,Math.sin(ang)*2);_addL(wl);}
  // עמודים
  [0,45,90,135,180,225,270,315].forEach(deg=>{
    const r=14,a=deg*Math.PI/180;_box(1.2,4.5,1.2,0x1a1c28,0x0a0c1a,Math.cos(a)*r,2.25,Math.sin(a)*r);
    const pl=new THREE.PointLight(0xffc844,2.5,12);pl.position.set(Math.cos(a)*r,4.5,Math.sin(a)*r);_addL(pl);
  });
  // בניינים ברקע
  [[-25,8,15],[25,12,10],[-20,6,-18],[20,10,-18],[0,15,-22]].forEach(([x,h,z])=>{
    _box(8,h,5,0x080a12,0,x,h/2,z);
    for(let wy=2;wy<h-1;wy+=3)for(let wx=-2;wx<=2;wx+=2){if(Math.random()>.3)_box(.7,1,.1,0x001133,0x002244,x+wx,wy,z+2.4);}
  });
  _rabinBoss=null;
}
function _spawnRabinBoss(){
  if(!_rabinScene)return;
  const g=new THREE.Group();
  const body=new THREE.Mesh(new THREE.BoxGeometry(1.4,1.8,1),new THREE.MeshLambertMaterial({color:0x0d0f1e,emissive:0x001133}));body.position.y=.9;g.add(body);
  const head=new THREE.Mesh(new THREE.BoxGeometry(.9,.9,.9),new THREE.MeshLambertMaterial({color:0x0d0f1e,emissive:0x001133}));head.position.y=2.15;g.add(head);
  const visor=new THREE.Mesh(new THREE.BoxGeometry(.85,.2,.1),new THREE.MeshLambertMaterial({color:0xff0000,emissive:0xcc0000}));visor.position.set(0,2.15,.46);g.add(visor);
  [-0.85,0.85].forEach(ox=>{const sh=new THREE.Mesh(new THREE.BoxGeometry(.6,.4,.6),new THREE.MeshLambertMaterial({color:0x111330,emissive:0x001144}));sh.position.set(ox,1.7,0);g.add(sh);});
...
function enterRabinSq(){
  G.paused=true;
  fadeOut(()=>{
    if(!_rabinScene)buildRabinScene();
    RABIN_SQ.inRabin=true;RABIN_SQ.playerX=0;RABIN_SQ.playerZ=18;RABIN_SQ.enterGrace=2;G.yaw=Math.PI;
    if(_rabinCam){_rabinCam.position.set(0,6,24);_rabinCam.lookAt(0,1,0);}
    if(taScene)taScene.remove(PB);else scene.remove(PB);
    _rabinScene.add(PB);PB.position.set(0,0,18);
    showN('🏛️ כיכר רבין. הכל מגיע לכאן.');
    setTimeout(()=>{if(RABIN_SQ.inRabin)_spawnRabinBoss();},1500);
    G.paused=false;fadeIn();
  });
}
function exitRabinSq(){
  G.paused=true;
  fadeOut(()=>{
    RABIN_SQ.inRabin=false;if(_rabinScene)_rabinScene.remove(PB);
    if(taScene)taScene.add(PB);else scene.add(PB);PB.position.set(30,0,-25);
    _rabinObjs.forEach(o=>{if(o.geometry)o.geometry.dispose();if(o.material){if(Array.isArray(o.material))o.material.forEach(m=>m.dispose());else o.material.dispose();}});
    _rabinObjs.length=0;_rabinBoss=null;_rabinScene=null;_rabinCam=null;
    G.paused=false;fadeIn();
  });
}
function updRabinSq(dt){
  if(!RABIN_SQ.inRabin||G.paused||G.dlgOpen||G.cutOpen)return;
  RABIN_SQ.enterGrace=Math.max(0,RABIN_SQ.enterGrace-dt);if(G.atkCD>0)G.atkCD-=dt;
  const spd=G.dogs[G.dog].spd;
  _vFwd.set(-Math.sin(G.yaw),0,-Math.cos(G.yaw));_vRgt.set(Math.cos(G.yaw),0,-Math.sin(G.yaw));
  let ix=0,iz=0;
  if(G.keys['KeyW']||G.keys['ArrowUp']){ix+=_vFwd.x;iz+=_vFwd.z;}
  ...
  if(G.joy.on){ix+=_vFwd.x*(-G.joy.dy)+_vRgt.x*G.joy.dx;iz+=_vFwd.z*(-G.joy.dy)+_vRgt.z*G.joy.dx;}
  const iln=Math.hypot(ix,iz)||1;
  let nx=RABIN_SQ.playerX+(ix/iln)*spd*dt,nz=RABIN_SQ.playerZ+(iz/iln)*spd*dt;
  nx=Math.max(-21,Math.min(21,nx));nz=Math.max(-21,Math.min(21,nz));
  RABIN_SQ.playerX=nx;RABIN_SQ.playerZ=nz;PB.position.set(nx,0,nz);
  if(Math.abs(ix)>.01||Math.abs(iz)>.01){walkT+=dt*8;dogLegs.forEach(lg=>{lg.node.rotation.x=Math.sin(walkT+lg.ph)*.38;});if(dogModel){const b=dogModel._baseY||.25;dogModel.position.y=b+Math.abs(Math.sin(walkT))*.09;}PB.rotation.y=Math.atan2(-ix,-iz);}
  ...
  if(_rabinBoss&&!_rabinBoss.dead){
    ...
    if(G.atkCD<=0&&G.keys['KeyF']&&dd<4){
      ...
      if(b.bar)b.bar.scale.x=Math.max(0,b.hp/b.mhp);
      if(b.hp<=0){b.dead=true;b.mesh.visible=false;sCapture();haptic([100,40,100,40,120]);addXP(500);G.score+=5000;G.coins+=300;updCoins();showN('🏆 המפקד הובס!');G._taBossMgr={dead:true};setTimeout(()=>exitRabinSq(),2000);}
    }
  }
}
...
function enterTelAviv(){
  ...
  if(taScene&&!_taSeaMesh)_buildTASea();
    ...
    if(!G._taLabRoofEntry)G._taLabRoofEntry={x:70, z:-114};
...
function exitTelAviv(){
  ...
  fadeOut(()=>{
    ...
    scene.add(PB);
    // ── החזר אורות main scene ──
    _mainPLights.forEach(l=>{ if(l&&l.isLight&&l._savedI!==undefined){l.intensity=l._savedI;} });
...
function updTelAviv(dt){
  ...
  if(G.keys['KeyF']&&G.atkCD<=0){G.atkCD=_taAtkCD;doAtk();}
  // תנועה
...
// מפה גדולה — תל אביב
// ════════════════════════════════════════════════
function drawBigMapTA(){
  const canvas=document.getElementById('mm-big');
  const S=Math.min(window.innerWidth,window.innerHeight)-40;
  if(canvas.width!==S){canvas.width=S;canvas.height=S;canvas.style.width=S+'px';canvas.style.height=S+'px';}
  const ctx=canvas.getContext('2d');
  const W=S,H=S;
  const px=TA.playerX+mapOffX, pz=TA.playerZ+mapOffZ;
  const sc=(S/350)*mapZoomLevel;
  const wx=x=>W/2+(x-px)*sc;
  const wz=z=>H/2+(z-pz)*sc;
  // רקע לילה עירוני
  ctx.fillStyle='#0d1520';ctx.fillRect(0,0,W,H);
  // ים — מצד מערב
  ctx.fillStyle='#0a2a45';
  ctx.fillRect(Math.max(0,wx(150)),0,W,H);
  // שכונות
  const zones=[
    {x:-40,z:-20,w:160,d:120,col:'rgba(200,185,150,0.18)',name:'בולבר'},
    {x:-120,z:100,w:150,d:170,col:'rgba(180,140,70,0.20)',name:'שוק הכרמל'},
    {x:-20,z:-150,w:220,d:140,col:'rgba(140,160,200,0.18)',name:'רמת אביב / צפון'},
    {x:165,z:-155,w:100,d:130,col:'rgba(90,100,120,0.28)',name:'נמל'},
    {x:-60,z:170,w:160,d:120,col:'rgba(190,155,90,0.20)',name:'יפו'},
    {x:0,z:-60,w:80,d:80,col:'rgba(120,200,120,0.15)',name:'כיכר רבין'},
  ];
  zones.forEach(z=>{
    ctx.fillStyle=z.col;
    ctx.fillRect(wx(z.x-z.w/2),wz(z.z-z.d/2),z.w*sc,z.d*sc);
    if(mapZoomLevel>=0.6){
      ctx.fillStyle='rgba(255,255,255,0.15)';ctx.font=`${Math.round(10*mapZoomLevel)}px sans-serif`;
      ctx.textAlign='center';ctx.fillText(z.name,wx(z.x),wz(z.z));
    }
  });
  // כבישים E-W
  const ewRoads=[
    {z:80,w:3,col:'#3a5050',name:''},
    {z:30,w:3,col:'#3a5050',name:'שד׳ בן גוריון'},
    {z:-30,w:3.5,col:'#445555',name:'רח׳ דיזנגוף'},
    {z:-80,w:3,col:'#3a5050',name:''},
    {z:-130,w:4,col:'#505A5A',name:'שד׳ ז׳בוטינסקי'},
    {z:-175,w:3,col:'#3a4a4a',name:''},
  ];
  ewRoads.forEach(r=>{
    ctx.strokeStyle=r.col;ctx.lineWidth=r.w;
    ctx.beginPath();ctx.moveTo(0,wz(r.z));ctx.lineTo(W,wz(r.z));ctx.stroke();
    if(r.name&&mapZoomLevel>=0.8){ctx.fillStyle='rgba(200,230,220,0.6)';ctx.font=`${Math.round(8*mapZoomLevel)}px sans-serif`;ctx.textAlign='left';ctx.fillText(r.name,wx(-200)+5,wz(r.z)-3);}
  });
  // כבישים N-S
  const nsRoads=[
    {x:-60,w:3,col:'#3a5050',name:''},
    {x:0,w:3,col:'#3a5050',name:''},
    {x:55,w:3.5,col:'#445555',name:'רח׳ אלנבי'},
    {x:115,w:3,col:'#3a5050',name:''},
    {x:-120,w:3,col:'#3a4a4a',name:''},
    {x:-180,w:3,col:'#3a4a4a',name:''},
  ];
  nsRoads.forEach(r=>{
    ctx.strokeStyle=r.col;ctx.lineWidth=r.w;
    ctx.beginPath();ctx.moveTo(wx(r.x),0);ctx.lineTo(wx(r.x),H);ctx.stroke();
    if(r.name&&mapZoomLevel>=0.8){ctx.fillStyle='rgba(200,230,220,0.6)';ctx.font=`${Math.round(8*mapZoomLevel)}px sans-serif`;ctx.textAlign='center';ctx.fillText(r.name,wx(r.x),20);}
  });
  // נקודות עניין
  const pois=[
    {pos:G._taMarketPos||{x:-90,z:100},col:'#ff8822',r:8,icon:'🏪',name:'שוק הכרמל'},
    {pos:G._taApexBldPos||{x:70,z:-130},col:'#cc44ff',r:8,icon:'🏢',name:'בניין APEX'},
    {pos:G._taPortPos||{x:168,z:-155},col:'#4488cc',r:8,icon:'⚓',name:'נמל'},
    {pos:G._taRabinPos||{x:0,z:-60},col:'#88ff44',r:7,icon:'🏛',name:'כיכר רבין'},
    {pos:G._taDizengoffSq||{x:-30,z:-20},col:'#44aaff',r:6,icon:'🎭',name:'כיכר דיזנגוף'},
    {pos:G._taNiraPos,col:'#ffcc00',r:5,icon:'🐕',name:'נירה'},
    {pos:G._taLabRoofEntry||{x:70,z:-114},col:'#00ff88',r:5,icon:'🔑',name:'כניסת גג'},
    {pos:G._taLabInteriorPos||{x:70,z:-148},col:'#00bbff',r:5,icon:'🔬',name:'מעבדה'},
  ];
  pois.forEach(p=>{
    if(!p.pos)return;
    ctx.fillStyle=p.col;ctx.beginPath();ctx.arc(wx(p.pos.x),wz(p.pos.z),p.r*Math.max(0.7,mapZoomLevel),0,Math.PI*2);ctx.fill();
    ctx.strokeStyle='rgba(255,255,255,0.5)';ctx.lineWidth=1.5;ctx.stroke();
    if(mapZoomLevel>=0.7){
      ctx.font=`bold ${Math.round(10*mapZoomLevel)}px sans-serif`;ctx.textAlign='center';
      ctx.fillStyle='rgba(0,0,0,0.65)';ctx.fillRect(wx(p.pos.x)-32*mapZoomLevel,wz(p.pos.z)-21*mapZoomLevel,64*mapZoomLevel,14*mapZoomLevel);
      ctx.fillStyle='#fff';ctx.fillText(p.name,wx(p.pos.x),wz(p.pos.z)-9*mapZoomLevel);
    }
  });
  // בניינים TA
  if(typeof _taBldList!=='undefined'){
    ctx.fillStyle='#253535';
    _taBldList.forEach(b=>ctx.fillRect(wx(b.x-b.w/2),wz(b.z-b.d/2),b.w*sc,b.d*sc));
  }
  // אויבים TA
  ctx.fillStyle='#ff3322';
  taEnemies.forEach(e=>{
    if(e.hp<=0||!e.mesh||!e.mesh.visible)return;
    ctx.beginPath();ctx.arc(wx(e.mesh.position.x),wz(e.mesh.position.z),5,0,Math.PI*2);ctx.fill();
  });
  // יעד GPS — מהבהב
  if(_navTargetWorld){
    const tx2=wx(_navTargetWorld.x),tz2=wz(_navTargetWorld.z);
    if(Math.sin(Date.now()*.006)>0){
      ctx.strokeStyle='#f5c518';ctx.lineWidth=2;const r2=7;
      ctx.beginPath();ctx.moveTo(tx2-r2,tz2-r2);ctx.lineTo(tx2+r2,tz2+r2);ctx.stroke();
      ctx.beginPath();ctx.moveTo(tx2+r2,tz2-r2);ctx.lineTo(tx2-r2,tz2+r2);ctx.stroke();
      ctx.beginPath();ctx.arc(tx2,tz2,r2+3,0,Math.PI*2);ctx.stroke();
    }
  }
  // שחקן
  const spx=W/2-mapOffX*sc,spz=H/2-mapOffZ*sc;
  ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(spx,spz,9,0,Math.PI*2);ctx.fill();
  ctx.strokeStyle='#f5c518';ctx.lineWidth=3;
  ctx.beginPath();ctx.moveTo(spx,spz);ctx.lineTo(spx-Math.sin(G.yaw)*18,spz-Math.cos(G.yaw)*18);ctx.stroke();
  // קואורדינטות
  ctx.fillStyle='rgba(0,0,0,.7)';ctx.fillRect(0,H-28,W,28);
  ctx.fillStyle='#88ddff';ctx.font='bold 12px sans-serif';ctx.textAlign='center';
  ctx.fillText(`🌆 תל אביב  📍 ${Math.round(TA.playerX)}, ${Math.round(TA.playerZ)}  •  גרור להזזה • זום לפינץ׳`,W/2,H-9);
  // מקרא
  if(mapZoomLevel>=0.6){
    ctx.font='11px sans-serif';ctx.textAlign='left';
    [['#fff','אתה'],['#ff3322','אויב APEX'],['#f5c518','יעד GPS'],['#00ff88','כניסת גג'],['#cc44ff','APEX'],['#4488cc','נמל']].forEach(([c,l],i)=>{
      ctx.fillStyle='rgba(0,0,0,.55)';ctx.fillRect(4,i*16+4,72,14);
      ctx.fillStyle=c;ctx.beginPath();ctx.arc(12,i*16+12,4,0,Math.PI*2);ctx.fill();
      ctx.fillStyle='#ddd';ctx.fillText(l,22,i*16+16);
    });
  }
...
function _checkTAMissionTriggers(){
  ...
  if(G.mission===54&&!G._ta54done){
    ...
    if(d2(px,pz,mp.x,mp.z)<30){
      ...
      const bx=0,bz=115;
      const _doTACinema=()=>{
        if(!taCamera){setTimeout(_doTACinema,200);return;} // המתן לטעינת taCamera
        _playCinema([
          {cam:[bx,    22, bz+40],  look:[bx, 0, bz],      dur:1200, note:'🌊 תל אביב... עיר אחרת. עולם אחר.'},
          {cam:[bx+20, 18, bz+20],  look:[bx, 2, bz-10],   dur:1400},
          {cam:[mp.x,  15, mp.z+35],look:[mp.x,1,mp.z],    dur:1200},
          {cam:[mp.x,   6, mp.z+15],look:[mp.x,0.5,mp.z],  dur:900}
        ],()=>{
  ...
  // 57 → גג בניין APEX → כניסה למעבדה (חדר פנים)
  ...
  if(G.mission===59&&!G._ta59done){
    ...
    if(d2(px,pz,pp.x,pp.z)<25){
      G._ta59done=true;
      G._portWHCleared=false;
      showCut('ch9_port_battle',()=>{
        setTimeout(()=>enterPortWH(),400);
      });
    }
    return;
  }
  // 60 → המחסן כבר נוקה בפנים (ר' _portWHCleared) → ממשיכים ישר לכיכר רבין
  // (התנאי הישן חיכה ל-taEnemies שבפועל לעולם לא מאוכלס במשימה הזו וגרם לתקיעות קבועה)
...
function updCh9(dt){
  ...
  if(G.mission===48&&!G._ch9_48done){
    ...
    G.mission=49;updateMissionHUD();updateNavArrow();saveGame();
    return; // מנע cascade לבלוק הבא באותו frame
  }
    ...
    if(d2(px,pz,-30,55)<8){
      ...
      setTimeout(()=>showCut('ch9_open',()=>{
        setTimeout(()=>showCut('ch9_messenger',()=>{
          ...
          G.mission=50;updateMissionHUD();updateNavArrow();saveGame();showN('📋 '+MISSIONS[50].txt);
        }),400);
      }),800);
    }
    return;
  ...
  if(G.mission===50&&!G._ch9_50done){
    if(d2(px,pz,105,25)<12){
      ...
      setTimeout(()=>showCut('ch9_zippo_reads',()=>{
        G.mission=51;updateMissionHUD();updateNavArrow();saveGame();showN('📋 '+MISSIONS[51].txt);
      }),600);
    }
    return;
  }
  if(G.mission===51&&!G._ch9_51done){
    if(d2(px,pz,205,-114)<14){
      ...
      setTimeout(()=>{G.mission=52;updateMissionHUD();updateNavArrow();saveGame();showN('📋 '+MISSIONS[52].txt);},800);
    }
    return;
  }
  if(G.mission===52&&!G._ch9_52done){
    if(d2(px,pz,205,-133)<14){
      ...
      setTimeout(()=>showCut('ch9_warehouse_map',()=>{
        G.mission=53;updateMissionHUD();updateNavArrow();saveGame();showN('📋 '+MISSIONS[53].txt);
      }),800);
    }
    return;
  }
  if(G.mission===53&&!G._ch9_53done){
    if(d2(px,pz,0,-150)<12){
      ...
      setTimeout(()=>showCut('ch9_z18_shadow',()=>{
        G.mission=54;updateMissionHUD();updateNavArrow();saveGame();showN('📋 '+MISSIONS[54].txt);
      }),1000);
    }
    return;
  }
