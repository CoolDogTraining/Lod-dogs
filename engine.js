...
function updPlayer(dt){
  ...
  if(dogModel&&dogModel._bipedalMode){
    // מצב דו-רגלי — רק הרגליים האחוריות מתנדנדות קצת
    if(moving){walkT+=dt*5;if(dogLegs[2])dogLegs[2].node.rotation.x=-.55+Math.sin(walkT)*.12;if(dogLegs[3])dogLegs[3].node.rotation.x=-.55+Math.sin(walkT+Math.PI)*.12;}
...
function _buildZippoLighter(){
  ...
  // פסים דקורטיביים בצדדים (עיטורי עלים)
  [-1,1].forEach(sx=>{const s=new THREE.Mesh(new THREE.BoxGeometry(.002,.14,.072),engM);s.position.set(sx*.06,0,0);g.add(s);});
  // חריטת "19" על הגוף הקדמי
  const n1=new THREE.Mesh(new THREE.BoxGeometry(.008,.08,.008),engM);n1.position.set(-.024,-.01,.036);g.add(n1);
  const n9c=new THREE.Mesh(new THREE.CylinderGeometry(.02,.02,.008,10),engM);n9c.rotation.x=Math.PI/2;n9c.position.set(.024,.02,.036);g.add(n9c);
  const n9t=new THREE.Mesh(new THREE.BoxGeometry(.008,.038,.008),engM);n9t.position.set(.034,-.01,.036);g.add(n9t);
  // כיסוי עליון פתוח
  ...
  // ציר
  const hinge=new THREE.Mesh(new THREE.CylinderGeometry(.008,.008,.075,6),goldD);hinge.rotation.z=Math.PI/2;hinge.position.set(0,.135,-.034);g.add(hinge);
  // גלגלת הצתה
  const wheel=new THREE.Mesh(new THREE.CylinderGeometry(.012,.012,.02,8),goldD);wheel.rotation.z=Math.PI/2;wheel.position.set(.01,.19,0);g.add(wheel);
  // להבה פנימית
  const flameMat=new THREE.MeshStandardMaterial({color:0xff9900,emissive:new THREE.Color(0xff5500),transparent:true,opacity:.9,roughness:.1});
  const flame=new THREE.Mesh(new THREE.ConeGeometry(.022,.065,6),flameMat);flame.position.y=.24;g.add(flame);
  // הילת להבה חיצונית
  const outerFlame=new THREE.Mesh(new THREE.ConeGeometry(.038,.055,6),new THREE.MeshStandardMaterial({color:0xffcc00,emissive:new THREE.Color(0xff8800),transparent:true,opacity:.35,roughness:.1}));
...
function _updateZippoLighter(){
  ...
  if(_zippoLighterMesh._outerFlame){
    ...
    _zippoLighterMesh._outerFlame.material.opacity=.25+Math.sin(t*13)*.15;
  }
...
function _showZippoLighter(){
  ...
  // זיפו עומד על שתיים רק כשהמצית נראית (משימה 32)
  if(dogModel&&G.dog==='zippo'){
    dogModel._bipedalMode=true;
    // הרם את גוף הכלב זקוף
    dogModel.position.y=(dogModel._baseY||0.25)+0.55;
    dogModel.rotation.x=-0.18; // הגוף נטוי קצת אחורה — טבעי לעמידה
    // הרגליים האחוריות (2,3) — ישרות למטה
    if(dogLegs[2])dogLegs[2].node.rotation.x=-0.55;
    if(dogLegs[3])dogLegs[3].node.rotation.x=-0.55;
    // הרגליים הקדמיות (0,1) — מורמות קדימה כאילו אוחזות
    if(dogLegs[0])dogLegs[0].node.rotation.x= 0.9;
    if(dogLegs[1])dogLegs[1].node.rotation.x= 0.9;
  }
}
function _hideZippoLighter(){
  if(_zippoLighterMesh)_zippoLighterMesh.visible=false;
  // חזרה לתנוחה רגילה על 4 רגליים
  if(dogModel){
    dogModel._bipedalMode=false;
    dogModel.rotation.x=0;
    dogModel.position.y=dogModel._baseY||0.25;
