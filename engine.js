...
function enterLab(){
  // תיקון באג: buildLabScene כבדה מאוד — בנה אותה לפני fadeOut כדי שלא תחסום את ה-thread
  // ובכך תמנע deadlock של G.paused=true ללא release
...
function updLab(dt){
  ...
  // גילוי ראשוני — mission 28 — מופעל פעם אחת עם כניסה
  if(G.mission===28&&!G._ch6LabVisited){
    G._ch6LabVisited=true;
    // תיקון באג: showCut מגדיר G.paused+G.cutOpen בעצמו — אל תגדיר G.paused כאן
    // כי זה יוצר deadlock: updLab לא רץ כש-paused=true, ואז showCut לא נקרא אף פעם
...
function updCh6(dt){
  ...
  if(G.mission===27&&!G._ch6PortVisited){
    ...
    // תיקון באג: בנה את labScene מראש בזמן שהשחקן מתקרב (לפני כניסה)
    // כדי שהבנייה הכבדה לא תקפיא את המשחק ברגע הכניסה
    if(!labScene&&d2(px,pz,25,-125)<30){
      buildLabScene();
    }
  }
