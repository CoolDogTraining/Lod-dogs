...
function _buildLabAsync(cb){
  if(labScene){cb();return;}
  const loadEl=document.createElement('div');
  loadEl.id='lab-loading';
  loadEl.style.cssText='position:fixed;inset:0;background:#030508;z-index:500;display:flex;align-items:center;justify-content:center;flex-direction:column;color:#00ff88;font-size:18px;font-family:monospace;';
  loadEl.innerHTML='<div>🔬 טוען מעבדה...</div><div style="font-size:12px;color:#006633;margin-top:8px">אנא המתן</div>';
...
function updLab(dt){
  ...
  // גילוי ראשוני — mission 28
  if(G.mission===28&&!G._ch6LabVisited){
    G._ch6LabVisited=true;
    // showCut מגדיר G.paused+G.cutOpen בעצמו — אין צורך ב-G.paused=true כאן
