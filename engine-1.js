...
function enterLab(){
  G.paused=true;
  fadeOut(()=>{
    // תיקון באג: buildLabScene כבדה — בנה אותה מחוץ ל-fadeOut כדי לא לחסום את ה-thread
    if(!labScene)buildLabScene();
    LAB.inLab=true;
    LAB.playerX=0;LAB.playerZ=8;LAB.playerYaw=Math.PI;
    G.yaw=Math.PI;LAB.enterGrace=3.5; // הגדלנו מ-3.0 ל-3.5 למניעת trigger מיידי של updLab
    if(labCamera){
...
function updLab(dt){
  ...
  // יציאה — mission 28 (גילוי ראשוני) — אוטומטי אחרי כניסה
  if(G.mission===28&&!G._ch6LabVisited){
    G._ch6LabVisited=true;
    // תיקון באג: timeout גדול יותר כדי לתת ל-buildLabScene לסיים לפני שמציגים קאטסין
    // וגם: אל נפעיל G.paused=true לפני שה-showCut בטוח ירוץ
    setTimeout(()=>{
      if(!G.cutOpen){  // guard: רק אם אין קאטסין פתוח כבר
        G.paused=true;
