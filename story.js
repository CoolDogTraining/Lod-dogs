...
const MISSIONS=[
  ...
  // 48 — פתיחת פרק ט׳: זיפו על הגג, כלב לבן מגיע
  {txt:'4️⃣8️⃣ פרק ט׳: חקרו את הכלב הלבן ברחוב הרצל 🤍',
   hint:'רחוב הרצל — כניסה לשכונה',
   targetFn:()=>({x:-30,z:55}),
   unlock:()=>{
     forceDog('zippo','זיפו מוביל — הכלב הלבן הגיע');
     setTimeout(()=>showCut('ch9_open',()=>{}),800);
   }},

  // 49 — קראו את המכתב מ-APEX, זיפו מחליט
  {txt:'4️⃣9️⃣ קראו את המכתב — הכנופייה מחליטה 📄',
   hint:'בסיס כלבי לוד',
   targetFn:()=>({x:105,z:25}),
   unlock:()=>{
     setTimeout(()=>showCut('ch9_messenger',()=>{}),600);
     setTimeout(()=>showCut('ch9_zippo_reads',()=>{}),4000);
   }},

  // 50 — חזרו למחסן APEX בלוד למצוא מפה
  {txt:'5️⃣0️⃣ חזרו למחסן APEX — יש שם עוד מידע 🗺️',
   hint:'מחסן APEX — אזור תעשייה',
   targetFn:()=>({x:205,z:-114}),
   unlock:()=>showN('🏭 המחסן נראה זנוח — אבל יש שם עוד.')},

  // 51 — מצאו את המפה עם סמני הערים
  {txt:'5️⃣1️⃣ מצאו את המפה בחדר הפנימי 📌',
   hint:'חדר פנימי במחסן',
   targetFn:()=>({x:215,z:-125}),
   unlock:()=>setTimeout(()=>showCut('ch9_warehouse_map',()=>{}),800)},

  // 52 — לכו לתחנת הרכבת — Z-18 מופיע כצל
  ...
  // 53 — כניסה לתל אביב — תחנת מרכז
  {txt:'5️⃣3️⃣ פרק ט׳: הגיעו לתל אביב מרכז 🌆',
   hint:'תל אביב — תחנה מרכזית',
   targetFn:()=>({x:0,z:-400}),
   unlock:()=>{
     setTimeout(()=>showCut('ch9_tel_aviv_arrive',()=>{}),1200);
     showN('🚆 הרכבת מגיעה. תל אביב מחכה.');
   }},
